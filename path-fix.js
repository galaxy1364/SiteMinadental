(() => {
  ...
  const scriptUrl = document.currentScript?.src || new URL('./path-fix.js', location.href).href;
  ...
  const resolve = (value) => {
    if (typeof value !== 'string' || !ROOT_ASSETS.has(value)) return value;
    return `${basePath}${value}` || value;
  };

  const originalSetAttribute = Element.prototype.setAttribute;
  Element.prototype.setAttribute = function(name, value) {
    if ((this instanceof HTMLImageElement || this instanceof HTMLSourceElement) && name.toLowerCase() === 'src') {
      value = resolve(value);
    }
