...
test("install invitation is non-blocking, accessible and conversion-ready", async () => {
  const [client, page] = await Promise.all([
    read("../app/site-client.tsx"),
    read("../app/page.tsx"),
  ]);
  assert.match(client, /INSTALL_NUDGE_DELAY = 12_000/);
  assert.match(client, /setInstallNudgeOpen\(true\)/);
  assert.match(client, /aria-describedby="install-description"/);
  assert.match(client, /event\.key === "Escape"/);
  assert.match(client, /event\.key !== "Tab"/);
  assert.match(client, /installReturnFocusRef\.current\?\.focus/);
  assert.match(client, /mina:conversion/);
  assert.match(client, /dataLayer\?\.push/);
  assert.match(client, /pwa_install_accept/);
  assert.match(`${client}\n${page}`, /data-conversion="appointment_whatsapp"/);
  assert.match(`${client}\n${page}`, /data-conversion="phone_call"/);
  assert.match(`${client}\n${page}`, /data-conversion="directions"/);
});

test("technical local SEO is crawl-ready without invented business facts", async () => {
  const [layout, page, config, robots, sitemap] = await Promise.all([
    read("../app/layout.tsx"),
    read("../app/page.tsx"),
    read("../app/site-config.ts"),
  ...
  assert.match(page, /Dentist/);
  assert.match(page, /دندانپزشکی در منطقه ۲۲ تهران/);
  assert.match(page, /mapEmbedUrl/);
  assert.match(page, /LiveRouteButton/);
  assert.match(config, /0x3f8de56cb097914d:0xfd5e3dc570462e50/);
  assert.match(robots, /Sitemap:/);
  assert.match(sitemap, /<urlset/);
  assert.doesNotMatch(`${layout}\n${page}`, /رتبه اول|بهترین دندانپزشک|تضمین/);
});
